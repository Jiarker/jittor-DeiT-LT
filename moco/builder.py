...
class NormedLinear(nn.Module):
    def __init__(self, num_classes=1000, feat_dim=2048):
        ...
        self.weight = nn.Parameter(jt.array((feat_dim, num_classes)))
...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.2, mlp=False, feat_dim=2048, normalize=False, num_classes=1000):
        ...
        # create the queue
        self.queue = jt.random((K, dim))
        self.queue = jt.normalize(self.queue, dim=0)
        self.queue_l = jt.randint(0, num_classes, (K,))
        self.queue_ptr = jt.zeros(1, dtype=jt.int32)
    ...
    def _hook_k(self, _, __, output):
        ...
        if self.normalize: 
           self.feat_after_avg_k = jt.normalize(self.feat_after_avg_k, dim=1)


    def _hook_q(self, _, __, output):
        self.feat_after_avg_q = flatten(output)
        if self.normalize:
           self.feat_after_avg_q = jt.normalize(self.feat_after_avg_q, dim=1)
    ...
    #     idx_shuffle = jt.randperm(batch_size_all)

    #     # broadcast to all gpus
    #     # nn.distributed.broadcast(idx_shuffle, src=0)

    #     # index for restoring
    #     idx_unshuffle = jt.argsort(idx_shuffle)

    #     # shuffled index for this gpu
    #     gpu_idx = jt.distributed.get_rank()
    ...
    def _train(self, im_q, im_k, labels):
        ...
        # compute logits
        features = jt.concat((q, k, self.queue.clone().detach()), dim=0)
        target = jt.concat((labels, labels, self.queue_l.clone().detach()), dim=0)
    ...
    def execute(self, im_q, im_k=None, labels=None):
        if self.training:
...
def concat_all_gather(tensor):
    ...
    *** Warning ***: torch.distributed.all_gather has no gradient.
    """
    # tensors_gather = [jt.ones_like(tensor)
    #     for _ in range(jt.distributed.get_world_size())]
    # torch.distributed.all_gather(tensors_gather, tensor, async_op=False)

    # output = jt.concat(tensors_gather, dim=0)
    output = jt.concat(tensor, dim=0)
    return output